...
def Home():
    # st.video('video.mp4')
    st.image('logo.png')
    st.title("Fast Transport Beijing Application")
    st.header("Data camp project")
...
def Feature_2():
    st.subheader(
        "Feature 2 : The average time needed to reach from point A to point B")

    with st.form(key='Feature_2'):
        a_longitude = st.number_input(
            "Longitude of point A", -180.0, 180.0, step=1e-10, format="%.9f", value=116.33772)
        a_latitude = st.number_input(
            "Latitude of point A", -90.0, 90.0, step=1e-10, format="%.9f", value=39.92724)
        b_longitude = st.number_input(
            "Longitude of point B", -180.0, 180.0, step=1e-10, format="%.9f", value=116.37429)
        b_latitude = st.number_input(
            "Latitude of point B", -90.0, 90.0, step=1e-10, format="%.9f", value=39.97052)
        date = st.date_input('Date', value=pd.to_datetime('2008-02-02'))
        time = st.time_input('Time', value=pd.to_datetime('14:19:00'))
    ...
    if submit_button2:
        ...
        client = openrouteservice.Client(
            key='5b3ce3597851110001cf6248b32018d70bf542d6bcb8f811f562cb17')
        ...
        for i in range(len(res["routes"][0]["segments"][0]["steps"])):
            ...
            if (res["routes"][0]["segments"][0]["steps"][i]["name"] != '-'):
                new_dict = {"distance": res["routes"][0]["segments"][0]["steps"][i]
                            ["distance"], "name": res["routes"][0]["segments"][0]["steps"][i]["name"]}
                itinerary.append(new_dict)

            else:
                if (len(itinerary) != 0):
                    itinerary[len(itinerary)-1]["distance"] = itinerary[len(itinerary) -
                                                                        1]["distance"] + res["routes"][0]["segments"][0]["steps"][i]["distance"]
        ...
        for i in range(len(itinerary)):
            ...
            road_density = df_road[(df_road['date_time'] > final_date) & (
                df_road['date_time'] < next_date)].shape[0]
        ...
        distance_txt = "<h5> <b>Distance :&nbsp" + "<strong>" + \
            str(round(res['routes'][0]['summary']['distance']/1000, 1)
                )+" Km </strong>" + "</h5></b>"
        duration_txt = "<h5> <b>Duration :&nbsp" + "<strong>" + \
            str(int(total_time/60)) + " Min </strong>" + "</h5></b>"

        m = folium.Map(location=[39.9075, 116.39723], zoom_start=10,
        ...
        folium.GeoJson(decoded).add_child(folium.Popup(
            distance_txt+duration_txt, max_width=200)).add_to(m)
...
if sidebar == 'Home':
    Home()
if sidebar == 'Feature 1':
    Feature_1()
if sidebar == 'Feature 2':
    Feature_2()
if sidebar == 'Feature 3':
    Feature_3()
